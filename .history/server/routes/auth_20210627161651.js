...
//Update Notification
router.get("/notifications/:id/:index", async(req, res) => {
    const notifications = await User.findByIdAndUpdate(req.body.id, {
        $set: { likedBy: req.body.userId },
    });
    res.json(notifications);
});
...
router.post("/updateprofilepic", async(req, res) => {
    await User.updateOne({ email: req.body.email }, {
        $set: { notification: req.body.profile },
    });
    const user = await User.findOne({ email: req.body.email });
