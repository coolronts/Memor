  ...
  export default {
    ...
    data(){
      return{
        ...
        style:{
          body:"mx-12 mb-12",
          buttonContainer:"max-w-1/3 flex items-center mr-3 text-white px-4 w-auto h-10 bg-red-600 rounded-full hover:bg-red-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none",
          button:"mr-1",
          ...
          title:"text-4xl max-w-full text-center ml-24 font-bold uppercase tracking-wide text-gray-800",
    ...
    methods:{
      ...
      async like(){
        if(this.isLogged){
          ...
          try {
              const response = await postService.updateLikes(this.post._id, this.$store.state.user._id,this.isLiked)
              .then(this.post = response)
            } catch (error) {
