          ...
          <div v-if="dropdownOpen" :class="style.notifications" style="width:20rem;">
            <div class="py-2">
              <div v-for="(notification, index) in notifications" :key="notification.post">
                <div @click="viewed(notification,index)" :class="[{'bg-gray-100':!notification.viewed} ,style.notificationContainer]">
...
<script>
  ...
  export default{
    ...
    data(){
     ...
     methods:{
      async  viewed(notification,index){
        const response = await axios.post('http://localhost:5000/updateNotification/'+ this.$store.state.user._id + index)
      }
    },
    created(){
      window.setInterval(() => {
        ...
        .then(this.notifications.forEach(async element => {
          ...
          if(this.postedUsers.length == this.notifications.length) {this.postedUsers = []}
        ...
        .then(this.notifications.forEach(async element => {
            ...
            if(this.postInfo.length == this.notifications.length) {this.postInfo = []}
