      ...
      <div :class="style.infoContainer">
        <div :class="style.heading">
          <div :class="style.buttonContainer">
            <button @click="showModal" :class="style.button" >Delete </button>
            <i class="far fa-trash-alt focus:outline-none"/>
          </div>
        ...
        <div :class="style.subHeading">
          ...
          <div :class="style.publish"> Published on: {{datePublished}}  </div>
        </div>
  ...
  export default {
    ...
    data(){
      return{
        ...
        modal:{
          openModal:false,
          heading:"Delete Post",
          subtext:"Are you sure you want to delete this post? All of your post's data will be permanently removed. This action cannot be undone."
        },
        style:{
          body:"mx-12 mb-12",
          buttonContainer:"outline-none ml-8 max-w-1/3 flex items-center mr-3 text-white px-4 w-auto h-10 bg-red-600 rounded-full hover:bg-red-700 active:shadow-lg mouse shadow transition ease-in duration-200 focus:outline-none",
          button:"mr-1 focus:outline-none",
    ...
    methods:{
      ...
      async setOption(choice){
        ...
        if(choice==0){ 
          await this.post.likedBy.map((userId)=>{
            await axios.post('/delete_notification'+ userId,{
              postId: this.post._id
            })
          }).then(await axios.delete('/delete_post/'+this.post._id))
