  ...
  export default{
    ...
    created(){
      ...
      window.setInterval(() => {
        ...
        axios.get('/notifications/'+ this.$store.state.user._id)
        .then(response => this.tempNotifications = response.data.notification)
        console.log("Notification:" this.notifications.length"tempNotifications:"this.tempNotifications.length)
        
          if(this.notifications.length != this.tempNotifications.length){
            
          console.log("hello")
          axios
          .get('/notifications/'+ this.$store.state.user._id)
          .then(response => ( this.notifications = response.data.notification))
          .then(this.$store.commit('setNotifications',this.notifications))
