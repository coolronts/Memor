...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async like(){
        !this.isLiked ? this.isLiked = true : this.isLiked = false
        try {
            const response = await axios.post('/updatelikes', {
            postId,
            userId,
            isLiked,
        });
             postService.updateLikes(this.postId, this.$store.state.user._id,this.isLiked)
