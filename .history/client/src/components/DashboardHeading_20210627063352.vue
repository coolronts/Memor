...
<div class="flex justify-center">
    <div  class="relative my-32 mr-24">
        ...
        <div v-if="dropdownOpen" class="border-2 absolute right-0 mt-2 bg-white rounded-md shadow-lg overflow-hidden z-20" style="width:20rem;">
         <div  class="py-2">
              <div v-for="notification in notifications" :key="notification.postId">
                <div v-for="postedUser in postedUsers" :key="postedUser._id">
                <a href="#" class="flex items-center px-4 py-3 border-b hover:bg-gray-100 -mx-2">
                 <img class="h-8 w-8 rounded-full object-cover mx-1" src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=334&q=80" alt="avatar">
                 <p class="text-gray-600 text-sm mx-2">
                     <span class="font-bold" href="#"> {{notification.postedName}} </span> replied on the <span class="font-bold text-blue-500" href="#">Upload Image</span> artical . 2m
                 </p>
                </a>
                </div>
              </div>
    ...
    created(){
      window.setInterval(() => {
        ...
        .then(this.notifications.forEach(element => {
              axios
                .get('http://localhost:5000/user/'+ element.user)
                .then(response => element.postedUsers = response)
              }))
        .then(this.notifications.forEach(element => {
              axios
                .get('http://localhost:5000/post/'+ element.post)
                .then(response => element.postInfo = response)
              }))
        .then(
          this.notifications.map((postedUser,i)=>{
             postedUser.image = this.postInfo[i].image }
          )
        )
        .then(this.notifications.forEach(element => {
                !element.viewed && a++
        ...
        .then(console.log(this.notifications))
