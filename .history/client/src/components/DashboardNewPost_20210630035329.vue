<template>
  <div :class="style.body">
    <div :class="style.container">
      ...
      <p :class="style.error" v-if="responseErrorMessage">{{responseErrorMessage}}</p>
      <p :class="style.success" v-if="responseSuccessMessage">{{responseSuccessMessage}}</p>
    ...
    data(){
      return{
        ...
        style:{
          ...
          error:"text-red-600 font-medium text-md text-center",
          success:"text-green-600 font-medium text-md text-center"
        },
    },
    methods: {
      validation(){
        if( !this.title || !this.place || !this.description || !this.imageBase64 ){
          this.responseErrorMessage = "Field Missing"
        }
        else{
          this.responseErrorMessage = "Data Missing"
        }
      },
      ...
      async postData(){
        try {
          ...
          .then(response => this.responseSuccessMessage = response.data)
