    ...
    <ul class="flex flex-col py-4">
      ...
      <li @click="switchDashboard(4)">
        <a href="#" class="flex flex-row items-center h-12 transform hover:translate-x-2 transition-transform ease-in duration-200 text-gray-500 hover:text-gray-800">
          ...
          <span class="ml-auto mr-6 text-sm bg-red-100 rounded-full px-3 py-px text-red-500"> {{notificationNumber}}  </span>
        </a>
      </li>
      <li>
  ...
  export default {
    ...
    mounted(){
      window.setInterval(() => {
        ...
        .then(response => ( this.notificationNumber = response.data.notification.length))
      }, 3000)
    }
 }
</script>
