          ...
          <div class="py-2">
            <div v-for="(notification, index) in notifications" :key="notification.post">
              <div :class="style.content">
                <img :class="style.img" :src="postedUsers[index].profile" alt="avatar">
                <p :class="style.secondary">
                    <span class="font-bold"> {{postedUsers[index].name}}  </span> replied on the <span class="font-bold text-blue-500" href="#">Upload Image</span> artical . 2m
                </p>
              </div>
            </div>
          </div>
    ...
    data(){
      return{
        style:{
          ...
          content:"flex items-center px-4 py-3 border-b hover:bg-gray-100 -mx-2",
          secondary:"text-gray-600 text-sm mx-2",
          img:"h-8 w-8 rounded-full object-cover mx-1",
