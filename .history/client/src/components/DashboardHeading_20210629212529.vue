  ...
  export default{
    ...
    created(){
      ...
      window.setInterval(() => {
        ...
        async const notifyChange = await axios.get('/notifications/'+ this.$store.state.user._id)
        this.tempNotifications = notifyChange
        console.log(this.notifications,this.tempNotifications)
        if(this.notifications.length != notifyChange.data.notification.length){
          axios
