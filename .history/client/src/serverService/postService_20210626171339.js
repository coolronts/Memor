...
const addLikes = "http://localhost:5000/updatelikes";
const removeLikes = "http://localhost:5000/updatelikes";
...
class postService {
    ...
    //Update Likes
    static updateLikes(postId, userId, likesCount, isLiked) {
        console.log(postId, userId, likesCount);
        if (isLiked) {
            return axios.post(addLikes, {
                postId,
                userId,
                likesCount,
            });
        } else {
            return axios.post(removeLikes, {
                postId,
                userId,
                likesCount,
            });
